...
const PROPERTY_UPDATE_TARGET_DAYS = Number(
  process.env["PROPERTY_UPDATE_TARGET_DAYS"]
)
...
async function updateGitHubIssues(tasks: Page[]) {
  for (const [index, task] of Object.entries(tasks)) {
    const state = PROPERTY_VALUE_STATUS_DONE.split(",").includes(task.status)
      ? "closed"
      : "open"
    ...
    console.log(
      "updated",
      updatedIssue.data.title,
      updatedIssue.data.number,
      updatedIssue.data.state
    )
